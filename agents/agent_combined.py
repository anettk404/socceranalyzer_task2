...
# Agent für Fragen die beide Datenquellen verbinden — erlaubt JOINs über
# openliga_* und statsbomb_* Tabellen hinweg (z.B. Tabellenstand + xG-Wert).
...
def combined_agent(state: GraphState) -> GraphState:
    ...
    Zwei LLM-Calls: erster generiert das SQL-Query mit möglichen Cross-Table-JOINs,
    zweiter formuliert das Datenbankergebnis als natürlichsprachige Antwort.
    """
    history_block = f"Gesprächskontext:\n{state.get('chat_history', '')}\n\n" if state.get("chat_history") else ""

    # 1. LLM-Call: SQL über alle vier Tabellen generieren, JOINs erlaubt
